...
function dynamicFileTable(fileInfo) {
	...
	//for(var i = 0; i < addFileArr.length; i++) {
		...
		if(checkFile(file)) {
			...
			$("#originalFileName").val(fileInfo.fileName);
			...
			.concat("<td class='text-left'><img src='/uploader/css/icon/" + fileExt + ".svg' class='file-icon' alt='' onerror='extNotExist(this);' />" + fileName + "</td>")
		...
		xhr.onreadystatechange = function(e) {
			if(xhr.readyState == XMLHttpRequest.DONE) {
				...
				if(result) {
					...
					$("#totalProgress").css("width", percentage.toFixed(1) + "%");
		...
		if(cookie.indexOf("file") >= 0) {
			for(var j = 0; j < fileArr.length; j++) {
				...
				if(cookieValue != "") {
					...
					if(jsonValue.fileName == fileInfo.fileName && jsonValue.fileSize == fileInfo.fileSize && jsonValue.loadCookie == false) {
